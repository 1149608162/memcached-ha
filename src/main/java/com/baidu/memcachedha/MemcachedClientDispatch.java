...
public class MemcachedClientDispatch {
	...
	private int syncThread = 16;

	private int reloadThread = 16;
		...
		try {
			...
			if (primaryKeeper != null && primaryKeeper.getStatus() == Stat.STAT_ONLINE) {
				...
				while ((response = primaryKeeper.invokeMethod(param)) == null && (retry <= this.primaryRetry)) {
					LOG.warn("do primary retry " + primaryKeeper.getIp() + ":" + primaryKeeper.getPort());
					retry++;
				}
			}
			...
			// breath not touch
			LOG.warn("breath not touch " + pex.getMessage() + " at " + primaryKeeper.getIp() + ":" + primaryKeeper.getPort());
		}

		List<MemcachedClientKeeper<?>> brokenPeers = new ArrayList<MemcachedClientKeeper<?>>(keepers.size());

		if (response == null && needSync) {
	...
	public class BreathLogic implements Runnable {
		...
		public void run() {
			try {
				if (LOG.isDebugEnabled()) {
					LOG.debug(("BreathLogic .. " + keeper.getIp() + ":" + keeper.getPort() + " stats : " + keeper.getStatus()));
				}
				...
				if (keeper.getStatus() == Stat.STAT_ONLINE) {
					LOG.error("server down " + keeper.getIp() + ":" + keeper.getPort());
					downKeeper(keeper);
				}
